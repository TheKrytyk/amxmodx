...
public plugin_init()
{
	...
	CvarAdminSlots[0] = create_cvar("amx_reservation", "0", FCVAR_PROTECTED, fmt("%L", LANG_SERVER, "CVAR_RESERVATION"));
	CvarAdminSlots[1] = create_cvar("amx_hideslots"  , "0", FCVAR_NONE     , fmt("%L", LANG_SERVER, "CVAR_HIDESLOTS"));
	
	hook_cvar_change(CvarAdminSlots[0], "@OnReservationChange");
	hook_cvar_change(CvarAdminSlots[1], "@OnHideSlotsChange");
...
public plugin_cfg()
{
	CvarReservation = get_pcvar_num(CvarAdminSlots[0]);
	CvarHideSlots = get_pcvar_num(CvarAdminSlots[1]);
}
